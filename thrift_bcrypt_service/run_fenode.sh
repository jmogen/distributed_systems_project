...
# Run frontend node for distributed bcrypt service
# Frontend node coordinates requests to backend nodes
...
sleep $TIMEA
echo Executing $0
ssh $SSH_OPTS -t $REMOTE_HOST "cd $current_dir;ls -l; timeout -s SIGKILL $TIMEC bash -c 'timeout -s SIGINT $TIMEB taskset -c $CORES $JAVA -Xmx${HEAPGB}g -cp .:gen-java/:\"lib/*\":\"jBCrypt-0.4/*\" FENode $PORT &> fenode.$REMOTE_HOST.$PORT.log'"
sleep 1
