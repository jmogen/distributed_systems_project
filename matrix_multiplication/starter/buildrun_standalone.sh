...
# Build and run matrix multiplication on a single node
# Standalone Matrix Multiplication Benchmark
...
for NUMPROCS in 4 16; do
	...
	/usr/bin/time timeout 5m $MPIRUN -np $NUMPROCS --host $HOSTNAME:16 ./matrix_multiplication.o $MTXORDER $INPUTA $INPUTB $OUTPUTC $RESULT $EXPNAME
	diff -b $GTC $OUTPUTC >> $STDOUTLOG 2>&1
	if [ $? -ne 0 ]; then
		echo Output is incorrect
		sed -i '$s/$/0/' $RESULT		
